...
def create_app(test_config=None):
    ...
    return jsonify({"success": True, "questions": formatted_search_results})
    ...
    if category is not None:
        ...
        if previous_question_ids:
            random_num = choice(
                [
                    n
                    for n in range(0, total_questions - 1)
                    if n not in previous_question_ids
                ]
            )
        else:
    ...
    def not_processable(error):
        return (
            jsonify({"success": False, "error": 422, "message": "Check input values"}),
            422,
        )
